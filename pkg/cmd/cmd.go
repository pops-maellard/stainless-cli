	...
	Command = &cli.Command{
		...
		Commands: []*cli.Command{
			...
			},
			{
				...
				UsageText:       "ee @manpages [-o ee.1] [--gzip]",
				Hidden:          true,
...
func generateManpages(ctx context.Context, c *cli.Command) error {
	...
	if c.Bool("text") {
		file, err := os.Create(filepath.Join(dir, "man1", "ee.1"))
		if err != nil {
			return err
		}
		defer file.Close()
	...
	if c.Bool("gzip") {
		file, err := os.Create(filepath.Join(dir, "man1", "ee.1.gz"))
		if err != nil {
			return err
		}
		defer file.Close()
