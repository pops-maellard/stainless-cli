...
The official CLI for the Ee REST API.
...
```sh
ee [resource] <command> [flags...]
```

```sh
ee store:orders create \
  --api-key 'My API Key' \
  --pet-id 1 \
  --quantity 1 \
  --status placed
...
```bash
ee <command> --arg @abe.jpg
```

Files can also be passed inside JSON or YAML blobs:

```bash
ee <command> --arg '{image: "@abe.jpg"}'
# Equivalent:
ee <command> <<YAML
arg:
  image: "@abe.jpg"
